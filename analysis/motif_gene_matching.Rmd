...
FC_motif_FC %>% ggplot(aes(x=`FC^2 GFP <-> RFP`)) + geom_density(fill='gray') + theme_minimal()
```

# Distribution of *relative* TFBS/motif enrichment *between* GFP <-> iPSC
```{r}
FC_motif_FC %>% ggplot(aes(x=`FC^2 GFP <-> iPSC`)) + geom_density(fill='gray') + theme_minimal()
```
