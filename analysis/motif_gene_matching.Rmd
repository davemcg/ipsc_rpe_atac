...
plotter <- function(motif) {
  ...
  geom_density_ridges() +
  geom_point(data = sample_motifs %>% filter(motif_alt_id == !!motif) %>% group_by(sample, motif_alt_id) %>% summarise(Count=n()) %>% ungroup(), aes(x=Count,y=sample), colour='red', size=2) +
...
# What TF/motifs are most enriched in GFP relative to RFP
