...
for (i in samples_full){
  sample <- str_split(str_split(i, '/')[[1]][6], '\\.')[[1]][1]
  input <- read_tsv(i)
...
for (i in bootstrap_stats){
  sample <- str_split(str_split(i, '/')[[1]][7], '\\.')[[1]][1]
  bootstrap_num <- str_split(str_split(str_split(i, '/')[[1]][7], '\\.')[[1]][2], '_')[[1]][2]
...
plotMA(resLFC)
plot(hist(resLFC$pvalue, breaks=1000))
...
gtf <- read_tsv("ftp://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_28/GRCh37_mapping/gencode.v28lift37.metadata.HGNC.gz", col_names = c('Transcript','Gene'))
...
```{r}
#samples_full = list.files('/Volumes/data/projects/nei/hufnagel/iPSC_RPE_ATAC_Seq/closest_TSS_motifs/processed/',
#                          pattern = '*dat.gz', full.names = T)

samples_full = list.files('/Volumes/McGaughey_S/closest_TSS_motifs/processed/',
...
sample_closestTSS %>% 
  ...
  summarise(Count=n(), motifs = paste(motif_loc, collapse=', ')) %>%
  group_by(Gene, Type) %>% 
  summarise(`Mean Count` = mean(Count), motifs = paste(motifs, collapse=',') ) %>% 
  arrange(-`Mean Count`)
```
