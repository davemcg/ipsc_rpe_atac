...
delta_cgm %>% mutate(center = `5000` + `1000` + `-1000` + `-5000`) %>% filter(!is.na(lTPM)) %>% group_by(center) %>% summarise(lTPM = mean(lTPM))
...
bayesglm <- train(log2FoldChange__RPE_vs_iPSC ~ ., data = train %>% select(Exon_Count:Gene_length, log2FoldChange__RPE_vs_iPSC), 
                 method = "bayesglm")

lm <- train(y = cgm_gfp %>% left_join(., expression %>% filter(Line == 'GFP'), by = c('Gene_Name' = 'Gene')) %>% filter(abs(lsTPM) > 0) %>% pull(lsTPM), x = cgm_gfp  %>% left_join(., expression %>% filter(Line == 'GFP'), by = c('Gene_Name' = 'Gene')) %>% filter(abs(lsTPM) > 0) %>% select(Exon_Count:Gene_length), 
