...
cgm_gfp <- read_tsv('/Volumes/data/projects/nei/hufnagel/iPSC_RPE_ATAC_Seq/CGM/GFP_common_peaks.blackListed.narrowPeak.CGM.tsv') %>% group_by(ENSGene_Name) %>% slice(1) %>% ungroup()
cgm_ipsc <- read_tsv('/Volumes/data/projects/nei/hufnagel/iPSC_RPE_ATAC_Seq/CGM/IPSC_common_peaks.blackListed.narrowPeak.CGM.tsv') %>% group_by(ENSGene_Name) %>% slice(1) %>% ungroup()
cgm_rfp <-  read_tsv('/Volumes/data/projects/nei/hufnagel/iPSC_RPE_ATAC_Seq/CGM/RFP_common_peaks.blackListed.narrowPeak.CGM.tsv') %>% group_by(ENSGene_Name) %>% slice(1) %>% ungroup()


delta_cgm <- (cgm_gfp %>% select(Exon_Count:`-1e6`)) - (cgm_ipsc %>% select(Exon_Count:`-1e6`))
delta_cgm$Gene_Name = cgm_gfp$Gene_Name

x <- delta_cgm %>% left_join(expression %>% filter(Line == 'GFP'), by = c('Gene_Name' = 'Gene'))


set.seed(153490)
train <- x %>% sample_n(10000) %>% filter(log2FoldChange__RPE_vs_iPSC > 0)
test <- x %>% filter(!Gene_Name %in% train$Gene_Name) %>% filter(log2FoldChange__RPE_vs_iPSC > 0)
...
fitControl <- trainControl(## 10-fold CV
                           ...
                           ## repeated ten times
                           repeats = 10)
bayesglm <- train(log2FoldChange__RPE_vs_iPSC ~ ., data = train %>% select(Exon_Count:`-1e6`, log2FoldChange__RPE_vs_iPSC), 
                 method = "bayesglm")

bridge <- train(log2FoldChange__RPE_vs_iPSC ~ ., data = train  %>% select(Exon_Count:`-1e6`, log2FoldChange__RPE_vs_iPSC), 
                 method = "bridge")

gamboost <-  train(log2FoldChange__RPE_vs_iPSC ~ ., data = train %>% select(Exon_Count:`-1e6`, log2FoldChange__RPE_vs_iPSC), 
                 method = "gamboost")

foba <-  train(log2FoldChange__RPE_vs_iPSC ~ ., data = train  %>% select(Exon_Count:`-1e6`, log2FoldChange__RPE_vs_iPSC), 
                 method = "foba")

brnn <-  train(log2FoldChange__RPE_vs_iPSC ~ ., data = train %>% select(Exon_Count:`-1e6`, log2FoldChange__RPE_vs_iPSC), 
                 method = "brnn")
```
