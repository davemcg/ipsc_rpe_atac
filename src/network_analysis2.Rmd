...
for (i in unique(TF_gene_stats_x$motif_core)){
  print(i)
  go_term_e <- enrichGO(gene          = TF_gene_stats_x %>% filter(motif_core == i) %>% pull(Gene),
                        keyType       = 'ENSEMBL',
                        OrgDb         = org.Hs.eg.db,
                        ont           = "all",
                        pAdjustMethod = "BH",
...
# rename clusters
cluster_motifs <- TF_gene_stats_n %>% group_by(Cluster) %>% summarise(motif_core = paste(sort(unique(motif_core)), collapse = ','))
TF_gene_stats <- TF_gene_stats %>% select(-motif_core) %>% left_join(cluster_motifs) %>% select(-motif) %>% unique()
