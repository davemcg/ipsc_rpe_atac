...
The core idea of this analysis is that the most significant HOMER identified TFBS are indeed enriched TFBS between GFP and RFP <-> iPSC and GFP <-> RFP. 
...
OK, now for a brief description of what HOMER is doing.

HOMER works on the peaks unique to GFP (and not RFP) and GFP and RFP (and not iPSC). 

More precisely, HOMER runs on the summits (highest peak intensity / read coverage) and the 100bp surrounding that. The sequenced is scanned against a randomized set of sequences and enriched motifs are identified. The document uses only the knownResults.html file (thus ignoring HOMER's attempt to ID novel motifs). This constricts identification of TFBS to TF with worked out motifs. 

After HOMER IDs TFBS, I then use HOMER again to scan the peak lists for the motifs that were ID'ed. That is the primary input for the document. 

The differential RNA-seq data from iPSC/RFP/GFP is also loaded in to do some more filtering. 
